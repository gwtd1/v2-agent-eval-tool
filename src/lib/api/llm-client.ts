 ...
 */
export function createTdLlmClient(apiKey?: string, baseUrl?: string): TdLlmClient {
  const envApiKey = process.env.TD_API_KEY;
  const envBaseUrl = process.env.TD_LLM_BASE_URL;
  ...
  return new TdLlmClient(finalApiKey, finalBaseUrl);
}

/**
 * Create TD LLM API client with runtime credentials (for testing)
 */
export function createTdLlmClientWithCredentials(apiKey: string, baseUrl?: string): TdLlmClient {
